...
void librtmpLogCallback(int level, const char *format, va_list vl){
	...
	//vsnprintf(ansiStr, strlen(ansiStr) - 1, format, vl);
	vsnprintf(ansiStr, 1024 - 1, format, vl);
	ansiStr[strlen(ansiStr) - 1] = 0;
...
int RTMP264_Connect(const char* url, RTMP** ppRtmp, int isOpenPrintLog, int logType){
	...
	if (RTMP_Connect(m_pRtmp, NULL) == FALSE){
		...
		printf("RTMP: Connect server succ...\n");
	}
	...
	if (RTMP_ConnectStream(m_pRtmp,0) == FALSE){
		...
		printf("RTMP: Connect stream succ...\n");
	}
	*ppRtmp = m_pRtmp;
	return true;
}
